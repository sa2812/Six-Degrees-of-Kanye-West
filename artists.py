    ...
    try:
        try:
            if (artist in ii.artists):
                artist_releases[ii.id] = [ii.title,
                                          [jj.name for jj in ii.artists],
                                          [{kk.title: [ll.id for ll in kk.artists]} for kk in ii.tracklist]]
        except AttributeError:
            if (artist in ii.main_release.artists):
                artist_releases[ii.id] = [ii.title,
                                          [jj.name for jj in ii.main_release.artists],
                                          [{kk.title: [ll.id for ll in kk.artists]} for kk in ii.tracklist]]
